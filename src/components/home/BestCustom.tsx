...
const BestCustom = () => {
    return (
        <section className='py-10 w-full mx-auto'>
            ...
            <main className='grid grid-cols-2 lg:grid-cols-3 gap-2  md:auto-rows-[200px] sm:gap-4  lg:gap-2'>
                {WorksGallery.slice(4,14)
                    .map((work,index) => (
                        <Link href={`/works/${work.title?.replaceAll(' ',"")}`}
                            key={index}
                            className={`relative h-56 lg:h-full md:col-span-${work.col} row-span-${work.row}`}
                        >
                            <Image
                                src={work.image}
                                alt={`work - ${index} `}
